...
// #include "../../clay.h"
// #include <SDL3/sdl.main.h> 
// #include <SDL3/SDL.h>
// #include <SDL3_ttf/sdl.ttf.h>
// #include <SDL3_image/sdl.image.h>
...
sdl_Clay_RenderFillRoundedRect :: proc(rendererData: ^Clay_SDL3RendererData, rect: sdl.FRect, cornerRadius: f32, _color: clay.Color) {
    ...
    vertices[vertexCount+0] = (sdl.Vertex){ {rect.x + clampedRadius, rect.y + clampedRadius}, color, {0, 0} }; //0 center TL
    ...
    for i :i32 = 0; i < numCircleSegments; i+=1 {
        ...
        for j: i32 = 0; j < 4; j+=1 {  // Iterate over four corners
            ...
            vertices[vertexCount+0] = (sdl.Vertex){ {cx + sdl.cosf(angle1) * clampedRadius * signX, cy + sdl.sinf(angle1) * clampedRadius * signY}, color, {0, 0} };
    ...
    indexCount += 6
    // Right edge
    vertices[vertexCount+0] = (sdl.Vertex){ {rect.x + rect.w, rect.y + clampedRadius}, color, {1, 0} }; //RT
    ...
    // Bottom edge
    vertices[vertexCount+0] = (sdl.Vertex){ {rect.x + rect.w - clampedRadius, rect.y + rect.h}, color, {1, 1} }; //BR
    ...
    // Left edge
    vertices[vertexCount+0] = (sdl.Vertex){ {rect.x, rect.y + rect.h - clampedRadius}, color, {0, 1} }; //LB
...
sdl_Clay_RenderArc :: proc(rendererData: ^Clay_SDL3RendererData, center: sdl.FPoint , radius: f32, startAngle: f32,  endAngle: f32,  thickness: f32,  color: clay.Color) {
...
sdl_Clay_RenderClayCommands :: proc(rendererData: ^Clay_SDL3RendererData, rcommands: ^clay.ClayArray(clay.RenderCommand))
{
    for i :i32= 0; i < rcommands.length; i+=1 {
        ...
        #partial switch (rcmd.commandType) {
            ...
            case .Text: {
                ...
                ttf.SetFontSize(font, f32(config.fontSize));
                text := ttf.CreateText(rendererData.textEngine, font, cstring(config.stringContents.chars), uint(config.stringContents.length));
                ttf.SetTextColor(text, u8(config.textColor.r), u8(config.textColor.g), u8(config.textColor.b), u8(config.textColor.a));
            ...
            case .Border: {
                ...
                if (config.cornerRadius.topLeft > 0) {
                    ...
                    sdl_Clay_RenderArc(rendererData, (sdl.FPoint){centerX, centerY}, clampedRadii.topLeft,
                ...
                if (config.cornerRadius.topRight > 0) {
                    ...
                    sdl_Clay_RenderArc(rendererData, (sdl.FPoint){centerX, centerY}, clampedRadii.topRight,
                ...
                if (config.cornerRadius.bottomLeft > 0) {
                    ...
                    sdl_Clay_RenderArc(rendererData, (sdl.FPoint){centerX, centerY}, clampedRadii.bottomLeft,
                ...
                if (config.cornerRadius.bottomRight > 0) {
                    ...
                    sdl_Clay_RenderArc(rendererData, (sdl.FPoint){centerX, centerY}, clampedRadii.bottomRight,
